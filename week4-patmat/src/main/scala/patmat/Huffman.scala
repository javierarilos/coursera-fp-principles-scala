...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    chars.groupBy(char => char).mapValues((chars: List[Char]) => chars.size).toList
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    (freqs.map(x => new Leaf(x._1, x._2))).sortBy((leaf) => leaf.weight)
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    trees match {
        case _ @ (_ :: Nil | Nil) => trees  // matching two cases: list size 1 or size 0
        case (first: CodeTree) :: (second: CodeTree) :: tail => makeCodeTree(first, second) :: tail
    }
  }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
    table.find((x) => x._1 == char).map(_._2).getOrElse(List[Bit]())
  }
  ...
  /**
    ...
    */
  def convert(tree: CodeTree): CodeTable = for(char <- chars(tree)) yield (char, encodeChar(tree)(char))
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    text.map((char) => codeBits(table)(char)).foldLeft(List[Bit]())((acc, aCharBits) => acc ++ aCharBits)
  }
}
